...
def FcCalc(trial_order, confoundorder=3):
    ...
    for n in range(len(trial_order)):
        for r in np.arange(1, confoundorder + 1):
            if n > (r - 1):
                Q[
                    cond_label_dict[trial_order[n]],
                    cond_label_dict[trial_order[n - r]],
                    r - 1,
                ] += 1
    ...
    for si in range(n_stimuli):
        for sj in range(n_stimuli):
            for r in np.arange(1, confoundorder + 1):
                Qexp[si, sj, r - 1] = (
                    P[si] * P[sj] * (n_trials - r)
                )  # JM edited this from Joke's code (ntrials +1? not ntrials-r)
    ...
    for i in range(nsim):
        ...
        if i == 0:
            ...
            cond_prob1_sum.iloc[:, cond_prob1_sum.columns != 'cond_val'] = (
                cond_prob1_sum.iloc[:, cond_prob1_sum.columns != 'cond_val']
                + cond_prob1_loop.iloc[:, cond_prob1_sum.columns != 'cond_val']
            )
            cond_prob2_sum.iloc[:, cond_prob2_sum.columns != 'cond_pairs'] = (
                cond_prob2_sum.iloc[:, cond_prob2_sum.columns != 'cond_pairs']
                + cond_prob2_loop.iloc[:, cond_prob2_sum.columns != 'cond_pairs']
            )
...
def calc_prob_next_given_last1(trial_order, cond_prob_given_last1_avg=None):
    ...
    for single in cond_list:
        single_locations = [
            m.start() for m in re.finditer(f'(?={single})', trial_order_str)
        ]
        after_single_location = np.array(single_locations) + 1
        after_single_location = after_single_location[
            after_single_location < len(trial_order)
        ]
        ...
        for cond in cond_list:
            if cond in cond_after:
                output_prob[f'pr({cond}|cond_val)'].append(
                    counts[cond_after == cond][0] / num_single
                )
    ...
    if cond_prob_given_last1_avg is not None:
        sum_abs_diff_w_avg_prob = np.sum(
            np.abs(
                cond_prob_given_last1_avg.drop(['cond_val'], axis=1)
                - output_prob_df.drop(['cond_val'], axis=1)
            ).values
        )
    else:
...
def calc_prob_next_given_last2(trial_order, cond_prob_given_last2_avg=None):
    ...
    for pair in all_pairs:
        pair_locations = np.array(
            [m.start() for m in re.finditer(f'(?={pair})', trial_order_str)]
        )
        ...
        if len(pair_locations) > 1:
            ...
            after_pair_location = after_pair_location[
                after_pair_location < len(trial_order)
            ]
            ...
            for cond in cond_list:
                if cond in cond_after:
                    output_prob[f'pr({cond}|cond_pairs)'].append(
                        counts[cond_after == cond][0] / num_pairs
                    )
                if cond not in cond_after:
    ...
    if cond_prob_given_last2_avg is not None:
        sum_abs_diff_w_avg_prob = np.sum(
            np.abs(
                cond_prob_given_last2_avg.drop(['cond_pairs'], axis=1)
                - output_prob_df.drop(['cond_pairs'], axis=1)
            ).values
        )
    else:
        sum_abs_diff_w_avg_prob = None
...
def pred_second_half_from_first(trial_order):
    ...
    for row in range(probs_last2_nopairs.shape[0]):
        ...
        if len(colnames_max) > 0:
            conds_max = [
                re.search(r'pr\(([a-z]*)|cond_pairs\)', val).group(1)
                for val in colnames_max
            ]
...
def est_eff_and_vif_old(
    events, tr, max_total_time, contrasts, time_past_last_offset=20, deriv=True
):
    """
    ...
    if deriv:
        ...
        hrf_model_val = 'spm'
    scan_cutoff = np.floor(
        (
            events.onset.values[-1:][0]
            + events.duration.values[-1:][0]
            + time_past_last_offset
        )
    )
    frame_times = np.arange(0, scan_cutoff, tr)
    desmat = make_first_level_design_matrix(
        frame_times, events, hrf_model=hrf_model_val, drift_model='cosine'
    )
    contrast_matrix = make_contrast_matrix(contrasts, desmat)
    cov_mat = (
        contrast_matrix
        @ np.linalg.inv(desmat.transpose() @ desmat)
        @ contrast_matrix.transpose()
    )
...
def est_psych_fitness(
    trial_type, avg_run_info, cond_prob_given_last1_avg, cond_prob_given_last2_avg
):
    """
    ...
    output['run_num_diff_from_avg'] = calc_run_num_diff_from_avg(
        trial_type, avg_run_info
    )
    _, output['prob_next_given_last1'], output['sum_abs_diff_prob_next_given_last1'] = (
        calc_prob_next_given_last1(trial_type, cond_prob_given_last1_avg)
    )
    _, output['prob_next_given_last2'], output['sum_abs_diff_prob_next_given_last2'] = (
        calc_prob_next_given_last2(trial_type, cond_prob_given_last2_avg)
    )
...
def run_eff_sim(
    nsim,
    events_inputs,
    make_timings_function,
    contrasts,
    avg_trial_repeats_info,
    tr,
    max_total_time,
    trials_psych_assess_map,
    cond_prob_given_last1_avg,
    cond_prob_given_last2_avg,
    ...
    for sim in range(nsim):
        ...
        if scan_cutoff > max_total_time:
            print(
                'WARNING:  You need to increase the total time to fit all trials \n'
                'estimates from this simulation set should be discarded'
            )
        eff_vals, vifs, _ = est_eff_and_vif(
            events,
            tr,
            max_total_time,
            contrasts,
