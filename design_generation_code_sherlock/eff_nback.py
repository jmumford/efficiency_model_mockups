...
def make_nback_timings(
    nblocks,
    n_act_trials_per_block,
    stim_dur,
    blank_dur,
    ...
    # choose block ordering
    block_type = np.random.choice([0, 1])
    if block_type == 0:
        block_type = ['one_back', 'two_back'] * n_each_back
    else:
        block_type = ['two_back', 'one_back'] * n_each_back
    ...
    for block in block_type:
        ...
        isi_vals_block, _ = sample_shifted_truncated_exponential(
            isi_exp_lam, isi_truncation, isi_shift, ntrials_block
        )
        fragment_durations_block = []
        for isi_val in isi_vals_block:
            fragment_durations_block.extend([isi_val, stim_dur, blank_dur])
        fragment_durations_block.extend(
            [break_fix_pre_message_dur, break_message_dur, break_fix_post_message_dur]
        )
        fragment_labels_block = (
            ['fixation', 'starter_trial', 'break'] * n_extra
    ...
    events_data = events_data.loc[
        events_data['trial_type'].str.contains('starter|back|break_message') == True
    ]
    return events_data
...
if __name__ == '__main__':
    ...
    events_inputs = {
        ...
        'stim_dur': 1,
        'blank_dur': 0.5,
        'break_fix_pre_message_dur': 0.5,
        'break_message_dur': 4,
        'break_fix_post_message_dur': 0.5,
        'isi_exp_lam': 2,
        'isi_truncation': 4.5,
        'isi_shift': 0.5,
    }
    ...
    unpermuted_trials = np.concatenate(
        [np.repeat('1', ntrials / 2), np.repeat('2', ntrials / 2)], axis=0
    )
    ...
    output_setting1.to_csv(
        f'/home/users/jmumford/efficiency/output/nback_output_{filenum}.csv'
    )
    with open(
        f'/home/users/jmumford/efficiency/output/nback_events_{filenum}.pkl', 'wb'
    ) as f:
