...
def make_visual_search_timings(
    n_con_high,
    n_con_low,
    n_feat_high,
    n_feat_low,
    ...
    for cur_block in range(nblocks):
        stim_type_block = np.random.permutation(
            np.concatenate(
                (
                    np.repeat(['con_high'], n_con_high_per_block),
    ...
    events_data = events_data.loc[
        events_data['trial_type'].str.contains('con|feat|break_message') == True
    ]
    return events_data
...
if __name__ == '__main__':
    ...
    unpermuted_trials = np.concatenate(
        [
            np.repeat('1', events_inputs['n_con_high'] / 3),
    ...
    output_setting1.to_csv(
        f'/home/users/jmumford/efficiency/output/visual_search_output_{filenum}.csv'
    )
    with open(
        f'/home/users/jmumford/efficiency/output/visual_search_events_{filenum}.pkl',
        'wb',
    ) as f:
