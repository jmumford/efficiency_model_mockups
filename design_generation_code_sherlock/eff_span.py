...
def make_span_timings(
    ...
    for cur_block in range(nblocks):
        for cur_trial in range(ntrials_per_block):
            fragment_durations.extend(
                [isi_vals[isi_count]] + 4 * [proc_dur, store_dur] + [recall_dur]
            )
            fragment_labels.extend(
                ['isi_fix']
                + ['first_proc_period', '_first_store_period']
                + 3 * ['proc_period', 'store_period']
                + ['recall']
            )
    ...
    events_data = events_data.loc[
        events_data['trial_type'].str.contains('first_proc|recall|break_message')
        == True
    ]
    events_data.loc[events_data['trial_type'].str.contains('proc'), 'duration'] = 4 * (
        proc_dur + store_dur
    )
    events_data.trial_type = events_data.trial_type.str.replace(
        'first_proc_period', 'emc'
    )
    return events_data
...
if __name__ == '__main__':
    ...
    contrasts = {'recall vs baseline': 'recall', 'emc vs baseline': 'emc'}
    ...
    output_setting1.to_csv(
        f'/home/users/jmumford/efficiency/output/span_output_{filenum}.csv'
    )
    with open(
        f'/home/users/jmumford/efficiency/output/span_events_{filenum}.pkl', 'wb'
    ) as f:
